            ...
            if current_count > last_count:
                ...
                for doc in cursor:
                    try:
                        ...
                        )  # Print the status regardless of its value
                        ...
                        if doc["status"] == "submitted":
                            ...
                            link = doc[
                                "link"
                            ]  # assuming 'link' is the field name for the meeting link
                            print(f"Link: {link}")

                            # Change the status of the document to 'processing'
                            result = Zoom_meeting_link.update_one(
                                {"_id": doc["_id"]}, {"$set": {"status": "processing"}}
                            )
                            ...
                            if result.modified_count > 0:
                                ...
                                subprocess.run(
                                    ["python", r"zoombot.py", link]
                                )  # replace with the path to your zoom bot file
                                ...
                                recorder_process = subprocess.run(
                                    ...
                                    capture_output=True,
                                )  # replace with the path to your recorder file
                                ...
                                if recorder_process.returncode == 0:
                                    ...
                                    recorded_file_path = (
                                        recorder_process.stdout.decode().strip()
                                    )

                                    # Insert the path of the recorded audio into MongoDB
                                    result = Zoom_meeting_link.update_one(
                                        {"_id": doc["_id"]},
                                        {"$set": {"recordedFile": recorded_file_path}},
                                    )
                                    if result.modified_count > 0:
                                        print(
                                            f"Inserted audio path into MongoDB: {recorded_file_path}"
                                        )
                                    else:
                                        print(
                                            "Failed to insert audio path into MongoDB."
                                        )
                                else:
