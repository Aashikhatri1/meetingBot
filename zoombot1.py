...
for image, sleep_time in zip(images, sleep_times):
    ...
    while True:
        ...
        if (best_confidence < 0.4 and image != 'zoombot_images\\line_2.png') or \
           (image == 'zoombot_images\\line_2.png' and best_confidence < 0.94):
...
while True:
    # Read the template image
    template = cv2.imread('zoombot_images\\line_2.png', cv2.IMREAD_UNCHANGED)
    template_gray = cv2.cvtColor(template, cv2.COLOR_BGR2GRAY)
    ...
    if best_confidence < 0.945:  # Adjust this threshold as needed
        print(f"'line_2.png' not found. Confidence: {best_confidence}")
