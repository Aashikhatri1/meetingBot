...
import boto3
ca = certifi.where()
from API_requests import send_post_request, get_diarisation_result, create_meeting_diarisation, get_all_indicator_list, create_indicator_diarisation
                        ...
                        if doc['status'] == 'submitted':
                            ...
                            if result.modified_count > 0:
                                ...
                                if recorder_process.returncode == 0:
                                    ...
                                    upload_diarisation = create_meeting_diarisation(id, diarisation)
                                    print('upload_diarisation', upload_diarisation)
                                    
                                    file_path = f"{id}.txt"
                                    try:
                                        with open(file_path,'r') as file:
                                            diarisation_str = file.read()
                                        print('File content:', diarisation_str)
                                    ...
                                    recording_analysis = gpt_response(indicators, diarisation_str)
                                    print('recording_analysis', recording_analysis)

                                    upload_analysis = create_indicator_diarisation(id, recording_analysis)
